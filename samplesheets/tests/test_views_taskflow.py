...
class TestIrodsObjectListAPIView(
        TestTaskflowBase, SampleSheetIOMixin, SampleSheetTaskflowMixin):
    """Tests for IrodsObjectListAPIView"""
    ...
    @skipIf(not TASKFLOW_ENABLED, TASKFLOW_SKIP_MSG)
    def test_get(self):
        """Test getting an assay dir"""
        irods_backend = get_backend_api('omics_irods')
        self.assertIsNotNone(irods_backend)

        with self.login(self.user):
            response = self.client.get(reverse(
                'samplesheets:irods_list',
                kwargs={
                    'assay': self.assay.omics_uuid,
                    'path': irods_backend.get_path(self.assay)}))

            self.assertEqual(response.status_code, 200)

    @skipIf(not TASKFLOW_ENABLED, TASKFLOW_SKIP_MSG)
    def test_get_invalid_path(self):
        """Test getting an assay dir with an invalid path kwarg"""
        irods_backend = get_backend_api('omics_irods')
        self.assertIsNotNone(irods_backend)
