...
class SheetTemplateCreateForm(forms.Form):
    ...
    def __init__(
        ...
        prompts = tpl_config.get('__prompts__')
    ...
    def save(self):
        ...
        with tempfile.TemporaryDirectory() as output_dir:
            if not isinstance(self.sheet_tpl, IsaTabTemplate):  # Custom tpl
                if not tpl_backend:
                    # NOTE: We shouldn't reach this point, but just in case..
                    raise ImproperlyConfigured(
                        'Isatemplates backend not enabled'
                    )
                tpl_backend.run_cookiecutter_custom(
                    self.sheet_tpl, output_dir, extra_context
                )
            else:  # CUBI template
                cookiecutter(
            ...
            path = os.path.join(output_dir, tpl_dir_name)
