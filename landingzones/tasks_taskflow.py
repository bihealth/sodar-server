...
class BaseLandingZoneStatusTask(SODARBaseTask):
    ...
    def set_status(
        ...
        if (
            landing_zone.status not in STATUS_BUSY
            and flow_name != 'landing_zone_create'
            and (file_count > 0 or landing_zone.status != 'MOVED')
        ):
            if app_alerts:
                try:
        ...
        member_notify = app_settings.get_app_setting(
            APP_NAME, 'member_notify_move', project=landing_zone.project
        )
        if member_notify and landing_zone.status == 'MOVED' and file_count > 0:
            members = list(
                set(
                    [
                        r.user
                        for r in landing_zone.project.get_all_roles()
                        if r.user != landing_zone.user
                    ]
                )
            )
        ...
        if status in ['MOVED', 'DELETED']:
            ...
            config_plugin = get_zone_config_plugin(landing_zone)
            if config_plugin:
                try:
                    config_plugin.cleanup_zone(landing_zone)
                except Exception as ex:
                    logger.error(
                        'Unable to cleanup zone "{}" with plugin '
                        '"{}": {}'.format(
                            landing_zone.title, config_plugin.name, ex
                        )
                    )
        ...
        if status == 'MOVED' and settings.SHEETS_ENABLE_CACHE:
            try:
                update_project_cache_task.delay(
                    project_uuid=str(landing_zone.project.sodar_uuid),
                    user_uuid=str(landing_zone.user.sodar_uuid),
                    add_alert=True,
                    alert_msg='Moved landing zone "{}".'.format(
                        landing_zone.title
                    ),
                )
            except Exception as ex:
